...
Commands and environment for running Mercury locally and in production. For the architecture
...
|---|---|---|
| `MERCURY_ENGINE` | Planner implementation selector: `fake` uses deterministic fixtures; every other value uses the real agent | real agent when unset; set `fake` for tests/demo |
| `MERCURY_SKIP_PLAN_ANCHORS` | Skip the entire plan gate | unset for `make demo` and live use; `1` only for reduced direct-server tests without grounding checkouts |
| `MERCURY_PORT` | HTTP listen port | `8090` |
| `MERCURY_BIND` | HTTP bind address | `127.0.0.1` (loopback — the bridge is the only client) |
| `MERCURY_SERVICE_TOKEN` | Bearer token between bridge and server | required for live; boot fails closed without it on non-loopback |
| `MERCURY_SERVICE_TOKEN_DASHBOARD` | Separate bearer for the dashboard BFF | required if the dashboard is deployed |
| `MERCURY_REQUIRE_AUTH` | Require a real token regardless of bind | baked `=1` in the container image |
| `MERCURY_REQUIRE_ENV_ONLY_TOKEN` | Refuse to boot if the file-fallback Jira token exists | baked `=1` in the container image |
| `MERCURY_LLM_PROVIDER` | Model provider used by the real agent: `anthropic` or `openrouter` | `anthropic` |
| `MERCURY_AGENT_MODEL` / `_DECOMPOSE` / `_GROOM` | Per-seat Claude model | `opus` |
| `MERCURY_AGENT_EFFORT` / `_DECOMPOSE` / `_GROOM` | Per-seat effort level | `xhigh` |
| `MERCURY_PLAN_BUDGET_USD` | Per-plan spend cap | `10` |
| `MERCURY_DAILY_SOFT_USD` | Daily soft cap (warns once per boot) | `50` |
| `MERCURY_DAILY_HARD_USD` | Daily hard cap (blocks new LLM work) | `100` |
| `MERCURY_RESULTS_DIR` | Plan state + spend ledgers root | `results` (container: `/data/results`) |
| `MERCURY_REPOS_ROOT` | Grounding repos root | `../grounding` (container: `/data/repos`) |
| `MERCURY_FETCH_BEFORE_PLAN` | fetch-before-plan: fetch + reset grounding repos before each plan | baked `=1` in the container |
| `MERCURY_COUPLING_MAP` | Coupling-map path shared by planner prompt and plan gate | `coupling-map.yaml` |

### Tracker writer

| Variable | Purpose | Default / required |
|---|---|---|
| `MERCURY_JIRA_TOKEN` | Jira API token — read by `tools/create-tree.mjs` only, stripped from the agent env | required for live; env-only on servers |
| `MERCURY_JIRA_CLOUD_ID` | Atlassian Cloud ID — the tracker write target | required for live; **no baked default** |
| `MERCURY_JIRA_PROJECT` | Jira project key | `PROJ` |
| `MERCURY_JIRA_SITE_URL` | Jira site URL for browse links | placeholder fallback (`https://your-domain.atlassian.net`); set it for useful live links |

### Grounding

| Variable | Purpose | Default / required |
|---|---|---|
| `MERCURY_GROUNDING_ORG` | GitHub org that owns the grounding repos | **required** — no baked default (a wrong default would clone a stranger's repos) |
| `MERCURY_GROUNDING_REPOS` | Comma-separated additions to the four fixed physical seed repos | unset = four seed checkouts; an explicit empty/malformed value is rejected; additions cannot remove seeds. The plan-only `cross-repo-lockstep` routing marker is never cloned (legacy entries are ignored) |
...
| `MERCURY_SLACK_POLL_MS` | Bridge poll cadence for watched plans | `7000` |

### Dashboard BFF

| Variable | Purpose | Default / required |
|---|---|---|
| `MERCURY_PLANNER_URL` | The planner service URL | required |
...
railway variable set MERCURY_JIRA_TOKEN --stdin
railway variable set MERCURY_JIRA_CLOUD_ID --stdin
railway variable set MERCURY_GROUNDING_ORG --stdin
railway variable set MERCURY_GROUNDING_REPOS --stdin
railway variable set GITHUB_TOKEN --stdin

# 4. live target values — set these explicitly; do not rely on the example fallbacks.
railway variable set MERCURY_JIRA_PROJECT --stdin
railway variable set MERCURY_JIRA_SITE_URL --stdin

# the service bearer is generated, never typed:
openssl rand -hex 32 | railway variable set MERCURY_SERVICE_TOKEN --stdin
...
service, expose the planner only on the provider's private mesh: set planner `MERCURY_BIND` to
a mesh-reachable bind address, keep the planner without a public domain, and point dashboard
`MERCURY_PLANNER_URL` at the private service URL. Configure a bridge bearer in planner
`MERCURY_SERVICE_TOKEN`, a different dashboard bearer in planner
`MERCURY_SERVICE_TOKEN_DASHBOARD`, and give the dashboard that same second value as
`MERCURY_SERVICE_TOKEN_DASHBOARD`. The two bearer values must be distinct so dashboard calls
...
1. `[mercury-entrypoint] volume ready: …` — `/data` is mounted and writable.
2. `[mercury-entrypoint] grounding repo <name>: cloned` — once per repo (first boot only;
   later boots log `fetched origin/main and reset the checkout to it`). A clone failure here
   is **fatal** — check `GITHUB_TOKEN` scope/expiry and `MERCURY_GROUNDING_ORG`.
3. `[mercury-supervise] server started` then
   `[mercury-supervise] server healthy — starting bridge`.
4. `[mercury] planner service listening on http://127.0.0.1:8090 (engine=real)`.
5. `[mercury-slack] Socket Mode connection established`.
...
- Everything operational goes to **stderr** (`[mercury-entrypoint]`, `[mercury-supervise]`,
  `[mercury-slack]` prefixes); the server's listening line is stdout. Railway captures both
...
- Wizard/language extraction, reconnects, poller warnings, surface-write refusals, and
  `[mercury] phase=…` timing lines are all stderr lines designed to make production incidents
...
- **Fail-closed boot.** `MERCURY_REQUIRE_AUTH=1` (baked in the image): a missing/empty
  `MERCURY_SERVICE_TOKEN` is fatal before the server listens. `/healthz` stays open
  (unauthenticated) for the supervisor; every `/plan*` route requires the bearer.
- **Env-only credentials.** `MERCURY_REQUIRE_ENV_ONLY_TOKEN=1` makes a file-fallback Jira
...
1. All required env vars set; `MERCURY_ENGINE` is unset or a real-mode value, not `fake`.
   Choose Anthropic or OpenRouter separately with `MERCURY_LLM_PROVIDER`.
2. `MERCURY_JIRA_CLOUD_ID` and `MERCURY_GROUNDING_ORG` set to **your** values — neither has a
   baked live default.
3. `MERCURY_JIRA_PROJECT` and `MERCURY_JIRA_SITE_URL` explicitly set to the real project and
...
7. Slack bridge pointed at the correct workspace; dashboard BFF (if deployed) pointed at the
   correct GitHub org.
8. `MERCURY_SKIP_PLAN_ANCHORS` is **unset** — production anchors must actually be checked.
...
- `GET /healthz` returns 200 with `version`, `engine`, `daily_spend_usd`, and
  ...
  route.
- `[mercury] phase=…` stderr lines per completed planning call carry `model`, `grounding`,
...
2. If rolling back the LLM provider: set `MERCURY_LLM_PROVIDER` to the previous value and
   `railway redeploy`.
