...
This guide takes you from a clean clone to a verified Radsvinn demo, then explains the
...
3. **Seed the coupling map** — the ground truth that makes Radsvinn coupling-aware.

Radsvinn's service boot fails closed for unsafe planner-API authentication and can enforce an
...
Radsvinn ships a deterministic **fake-mode demo** that replays known-good fixtures through the
...
`RADSVINN_LLM_PROVIDER=openrouter` and set `RADSVINN_OPENROUTER_API_KEY`; setting the key alone
...
```bash
export RADSVINN_JIRA_SITE_URL=https://your-domain.atlassian.net   # explicit live site; links: ${SITE}/browse
export RADSVINN_JIRA_CLOUD_ID=<your-atlassian-cloud-id>           # REQUIRED, no default
export RADSVINN_JIRA_PROJECT=PROJ                                 # explicit live project (PROJ is the fallback)
export RADSVINN_JIRA_TOKEN=<jira-api-token>                       # required for network operations
```
...
Grounding is how Radsvinn verifies that a ticket's code anchors actually exist. In v0.1.0,
...
`RADSVINN_REPOS_ROOT`. The plan/gate vocabulary also accepts `cross-repo-lockstep`, but that is
...
```bash
export RADSVINN_GROUNDING_ORG=example-org
export RADSVINN_GROUNDING_REPOS=web-app,api-service,worker-service,shared-lib
export RADSVINN_REPOS_ROOT=../grounding        # where the checkouts live (container: /data/repos)
...
single-container deploy does this automatically; locally, set `RADSVINN_FETCH_BEFORE_PLAN=1` or
keep the checkouts current yourself.

### 2d. Boot live

```bash
export RADSVINN_SERVICE_TOKEN=<random-bearer>   # every /plan* route now requires it
...
The coupling map (`coupling-map.yaml`, or `RADSVINN_COUPLING_MAP=<path>`) is the ground truth
that makes Radsvinn coupling-aware. It describes your system's hazards so the planner routes
around them and the gate can check that it did.

**An explicitly empty map is legal, but degraded.** With zero no-go zones, Radsvinn has no
...
map before trusting Radsvinn with live planning decisions.
...
|---|---|---|
| `RADSVINN_BIND` | `127.0.0.1` | Bind address (loopback in-container; the bridge is its only local client) |
| `RADSVINN_PORT` | `8090` | Listen port |
| `RADSVINN_ENGINE` | `real` | `fake` = deterministic fixture engine; anything else = the real agent |
| `RADSVINN_SERVICE_TOKEN` | unset | If set, every `/plan*` route requires `Authorization: Bearer <token>`; `/healthz` stays open |
| `RADSVINN_REQUIRE_AUTH` | unset | `1` makes a missing token FATAL at boot regardless of bind |
| `RADSVINN_REQUIRE_ENV_ONLY_TOKEN` | unset | `1` makes a present local Jira-token file FATAL at boot (server posture) |
| `RADSVINN_JIRA_SITE_URL` | `https://your-domain.atlassian.net` | Jira site for browse links; override the placeholder for live use |
| `RADSVINN_JIRA_CLOUD_ID` | *(required, no default)* | Your Atlassian Cloud ID; Jira network operations fail clearly if unset |
| `RADSVINN_JIRA_PROJECT` | `PROJ` | Jira project key the tree is created under |
| `RADSVINN_JIRA_TOKEN` | *(env / secrets; local file fallback)* | Jira API token — required by and used only by the deterministic writer; use env-only posture on servers |
| `RADSVINN_GROUNDING_ORG` | *(required, grounding)* | VCS org owning the grounding repos (example: `example-org`) |
| `RADSVINN_GROUNDING_REPOS` | four physical seed repos | Additive physical clone list; the separate `cross-repo-lockstep` plan-routing marker is never cloned (legacy env entries are ignored) |
| `RADSVINN_REPOS_ROOT` | *(local: a sibling dir)* | Where the grounding checkouts live; the container sets `/data/repos` |
| `RADSVINN_FETCH_BEFORE_PLAN` | unset | `1` runs `git fetch origin main` in every grounding repo before Break-Down |
| `RADSVINN_COUPLING_MAP` | `coupling-map.yaml` | Path to the coupling ground truth used by both Break-Down and the deterministic plan gate |
| `RADSVINN_ENGINE` / `ANTHROPIC_API_KEY` | `real` / unset | Engine selector plus the default Anthropic-provider key; `RADSVINN_ENGINE` does not select the LLM provider |
| `RADSVINN_LLM_PROVIDER` / `RADSVINN_OPENROUTER_API_KEY` | `anthropic` / unset | Provider selector plus OpenRouter key; the key alone does not switch providers |
| `RADSVINN_AGENT_MODEL` / `RADSVINN_AGENT_EFFORT` | `opus` / `xhigh` | Shared model/effort for both planning phases; `opus` is a floating CLI alias, not a pinned exact model ID |
| `RADSVINN_AGENT_MODEL_DECOMPOSE` / `_GROOM` | unset | Per-seat model override (decompose may be cheaper; groom stays strongest) |
| `RADSVINN_AGENT_PERMISSION_MODE` | `default` | Agent posture — keep `default` (the sandbox); do not weaken |
| `RADSVINN_AGENT_ALLOWED_TOOLS` | `Read,Grep,Glob` | Agent tool allowlist — read-only; do not add `Bash`/`Write` |
| `RADSVINN_LIGHT_MAX_TURNS` | `12` | `--max-turns` bound for light-grounding decompose calls |
| `RADSVINN_PLAN_BUDGET_USD` | `10` | Per-plan spend cap; checked before every Groom call and before a paid Phase-1 regeneration, never Create |
| `RADSVINN_DAILY_SOFT_USD` / `RADSVINN_DAILY_HARD_USD` | `50` / `100` | Current UTC-day spend — warn / refuse; resets at UTC midnight |
| `RADSVINN_RESULTS_DIR` | `results` | Where plan state + artifacts are written (gitignored) |
| `RADSVINN_TREECHECK_BIN` | unset | Path to a prebuilt `treecheck`; falls back to `go run ./cmd/treecheck` |
| `RADSVINN_SKIP_PLAN_ANCHORS` | unset | Keep unset for `make demo` and live use; `1` is only for reduced direct-server tests without grounding checkouts |
...
| `RADSVINN_SERVICE_URL` | `http://127.0.0.1:8090` | Base URL of the planner service this bridge drives |
| `RADSVINN_SERVICE_TOKEN` | unset | Match the planner's token so the bridge's calls are authorized |
| `RADSVINN_SLACK_POLL_MS` | `7000` | How often the bridge polls each watched plan |
...
| `RADSVINN_SERVICE_TOKEN_DASHBOARD` | *(required)* | Bearer token to the planner (blank fails boot) |
...
The team slugs default to Radsvinn-branded names — create those teams in your org, or point the
...
- **Never** put tokens (`RADSVINN_JIRA_TOKEN`, `ANTHROPIC_API_KEY`, GitHub secrets, Slack
...
- On a server, the Jira credential must be **env-only**; a local convenience file is rejected
  at boot when `RADSVINN_REQUIRE_ENV_ONLY_TOKEN=1` (set by the container).
